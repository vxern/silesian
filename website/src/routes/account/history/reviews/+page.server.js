...
export const load = async ({ locals }) => {
  const [reviewCount, reviewCountByMonth, reviewHistory] = await Promise.all([
    getReviewCount(locals.session),
    getReviewCountByMonth(locals.session),
    getReviewHistory(),
  ]);
...
function getReviewCount(session) {
  return db
    .select({ count: count() })
    .from(reviews)
...
async function getReviewCountByMonth(session) {
  const results = await db
    ...
    .where(eq(reviews.reviewer_id, session.user.id))
    .groupBy(sql`month`);
...
function getReviewHistory() {
  // TODO(vxern): This needs to get chucked out.
  return db
